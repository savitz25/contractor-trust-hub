...
| `entities` | Corporate shells (Sunbiz, etc.) |
| `contractor_entities` | Many-to-many link with role (qualifier, officer, DBA) |
| `discipline_actions` | Board / ULA / recovery-fund rows |
| `permit_events` | Local permit activity (later) |
| `trust_scores` | Transparent component scores + explanation JSON |
| `ingest_batches` | Provenance for every load |
