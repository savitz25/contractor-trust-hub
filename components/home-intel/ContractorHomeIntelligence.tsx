    ...
    <div className="cth-intel-home">
      ...
      <section id="states" aria-labelledby="explore-title">
        ...
        <ul className="cth-intel-geo">
          ...
          {network.stateCapabilities.filter(s => s.route && !intel.geography.some(g => g.code === s.state)).map(s => <li key={s.state}><p><strong>{s.state}</strong></p><p>Accepted state licensing or registration evidence, with source-specific scope.</p><p>Credential numbers are not unique companies. Evidence coverage does not assert specialist completion.</p><Link href={s.route!}>Explore state intelligence</Link></li>)}
        </ul>
