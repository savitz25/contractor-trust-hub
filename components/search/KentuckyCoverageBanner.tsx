...
import { KY_DHBC_SEARCH_URL } from "@/lib/states/ky-dhbc";
...
export function KentuckyCoverageBanner({ compact = false, showFloridaLink = true }: Props) {
  if (compact) {
    return (
      <div
      ...
      >
        <p className="text-xs font-semibold leading-snug sm:text-[13px]">
          Kentucky DHBC specialty credentials — not a statewide GC
        </p>
        <p className="mt-1 text-[11px] leading-relaxed text-sky-900/85 sm:text-xs">
          Electrical, HVAC, and plumbing contractor types from the official DHBC list. Missing here
          is not proof someone is unlicensed. No bond, insurance, or discipline invented.
        </p>
      </div>
    );
  }
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold leading-snug sm:text-[15px]">
            Kentucky does not issue a statewide general contractor license
          </p>
          <p className="mt-1.5 text-xs leading-relaxed text-sky-950/90 sm:text-sm">
            This search uses official{" "}
            <strong className="font-semibold">DHBC contractor-level specialties</strong>: Electrical
            (Contractor Electrician-Business), HVAC (Master HVAC Contractor), and plumbing (Master
            Plumber). Many general builders are city/county only. Always confirm on the official
            DHBC search.
          </p>
          <div className="mt-3 grid gap-2 sm:grid-cols-2 sm:gap-3">
            <div className="rounded-xl border border-sky-200/70 bg-white/70 px-3 py-2.5">
              ...
              <p className="mt-1 text-xs leading-relaxed text-sky-950/90 sm:text-[13px]">
                Published type, Active status, DBA when present, issued / expiration dates
              </p>
            </div>
            <div className="rounded-xl border border-sky-200/70 bg-white/70 px-3 py-2.5">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-sky-800/80">
              ...
              <p className="mt-1 text-xs leading-relaxed text-sky-950/90 sm:text-[13px]">
                Live status on official DHBC lookup. This list does not publish bond, insurance, or
                discipline.
              </p>
            </div>
          </div>
