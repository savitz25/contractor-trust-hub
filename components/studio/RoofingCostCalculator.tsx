...
import {
  computeRoofingEstimate,
  defaultRoofingCalcInput,
  formatUsd,
  getRoofingCalculatorConfig,
  type RoofingCalcInput,
  type RoofingOption,
} from "@/lib/plan/roofing-calculator";
...
export function RoofingCostCalculator() {
  const config = useMemo(() => getRoofingCalculatorConfig(), []);
  const [input, setInput] = useState<RoofingCalcInput>(() => defaultRoofingCalcInput());
  const [zip, setZip] = useState("");
  const [city, setCity] = useState("");

  const estimate = useMemo(() => computeRoofingEstimate(input), [input]);

  const set = <K extends keyof RoofingCalcInput>(key: K, id: string) => {
    setInput((s) => ({ ...s, [key]: id }));
  };
      ...
      <div className="space-y-5">
        <section className="rounded-3xl border border-[var(--border)] bg-white p-4 shadow-[var(--shadow-md)] sm:p-6">
          ...
          <h2 className="mt-1.5 text-lg font-semibold text-[var(--text)] sm:text-xl">
            Size, system, and access
          </h2>
          <p className="mt-1 text-sm text-[var(--muted)]">
            Adjust the main Florida cost drivers. Values update the planning band live — still not a
            bid.
          </p>

          <div className="mt-5 space-y-5">
            <OptionGroup
              legend="Approximate roof size"
              help="Use home size as a rough proxy if you do not know square count."
              options={config.sizeOptions}
              value={input.sizeId}
              onChange={(id) => set("sizeId", id)}
            />
            <OptionGroup
              legend="Roof material / system"
              options={config.materialOptions}
              value={input.materialId}
              onChange={(id) => set("materialId", id)}
            />
            <OptionGroup
              legend="Stories / access"
              options={config.storiesOptions}
              value={input.storiesId}
              onChange={(id) => set("storiesId", id)}
            />
            <OptionGroup
              legend="Pitch / complexity"
              options={config.pitchOptions}
              value={input.pitchId}
              onChange={(id) => set("pitchId", id)}
            />
          </div>
        </section>
      ...
      <div className="space-y-5 lg:sticky lg:top-28">
        <section className="overflow-hidden rounded-3xl border-2 border-[var(--navy)]/10 bg-white shadow-[var(--shadow-lg)]">
          <div className="border-b border-[var(--border)] bg-[var(--navy)] px-4 py-3 sm:px-6">
            ...
            <h2 className="mt-2 text-lg font-semibold text-white sm:text-xl">
              Florida roof replacement planning range
            </h2>
            <p className="text-sm text-white/75">
              {estimate.scaleLabel} · {estimate.material.label}
            </p>
          </div>
        ...
        <section className="rounded-3xl border border-[var(--border)] bg-white p-4 shadow-[var(--shadow-sm)] sm:p-6">
          ...
          <p className="mt-4 text-xs text-[var(--muted)]">
            ...
            {" · "}
            Factors:{" "}
            <code className="rounded bg-[var(--bg)] px-1 text-[11px]">roofing-calculator.json</code>
          </p>
          <p className="mt-2 text-xs text-[var(--muted)]">
            Prefer multi-trade Cost Studio?{" "}
