-- Contractor Trust Hub — Phase 0 initial schema
-- Target: PostgreSQL 15+
-- Refined against real FL DBPR CONSTRUCTIONLICENSE_1.csv layout (2026)
...
CREATE INDEX IF NOT EXISTS contractors_name_trgm_ready_idx ON contractors (display_name);
...
CREATE TABLE IF NOT EXISTS licenses (
  ...
  source_board            TEXT,                    -- e.g. CILB / board 06
  external_key            TEXT NOT NULL,           -- full license id CBC015082
  occupation_code         TEXT NOT NULL,           -- CBC, CGC, QB, ...
  ...
  status_normalized       TEXT,                    -- active | inactive | other | unknown
...
-- ---------------------------------------------------------------------------
-- Corporate entities (Sunbiz / SOS — Phase 1 fill)
-- ---------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS entities (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  source_system   TEXT NOT NULL,          -- fl_sunbiz, ...
...
CREATE TABLE IF NOT EXISTS contractor_entities (
  ...
  role            TEXT NOT NULL DEFAULT 'linked',  -- qualifier | officer | dba | linked
...
CREATE TABLE IF NOT EXISTS discipline_actions (
  ...
  classification      TEXT,                -- Licensed Activity, ...
  entered_date        DATE,
  disposition         TEXT,
...
COMMENT ON COLUMN licenses.status_normalized IS 'Derived: active | inactive | other | unknown';
