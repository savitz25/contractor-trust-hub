...
export type MaDolEvent = MassachusettsContractorSnapshot["dol_discipline"]["events"][number];
...
export function assertMassachusettsSnapshot(
  ...
  if (d.events.length !== d.rows) throw new Error("DOL event rows must match the count");
  if (d.coverage !== "PARTIAL") throw new Error("DOL discipline window is partial");
  if (d.events.some((e) => !["EL", "PL", "GF", "SM"].includes(e.boardCode))) throw new Error("Only construction-trade boards");
  ...
  for (const family of [d, value.dcamm_debarment, value.ag_fair_labor_debarment]) {
    ...
    if (family.events.some((e) => e.attribution !== "standalone_event_no_profile_join")) throw new Error("Events stay standalone");
  }
...
export function findMaDolEvents(license: string, boards?: readonly string[]): MaDolEvent[] {
  ...
  return MASSACHUSETTS_SNAPSHOT.dol_discipline.events.filter(
    (e) =>
