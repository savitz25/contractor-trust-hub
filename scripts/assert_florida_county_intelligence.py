...
def main() -> int:
    ...
    assert_("isFloridaCountyIntelSlug" in page, "county intel slug gate")
    assert_('"miami-dade"' in cat and 'dbprCountyCode: "23"' in cat, "Miami-Dade county_code 23")
    assert_('"pinellas"' in cat and 'dbprCountyCode: "62"' in cat, "Pinellas county_code 62")
    assert_("Confirmed Miami-Dade County-issued permit records" in payload, "MDC confirmed permit label")
    assert_("Not all Miami-Dade permits" in payload, "not all MDC permits")
    assert_("Issued is not final" in payload, "issued not final")
    ...
    assert_("Permit and local credential exports are still being acquired" in cat, "hero disclosure")
