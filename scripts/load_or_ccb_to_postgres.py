#!/usr/bin/env python3
"""
Load staged Oregon CCB Active Licenses into Postgres.
...
Usage:
  ...
  python scripts/load_or_ccb_to_postgres.py --staging-dir data/staging/or_ccb --limit 1000
  python scripts/load_or_ccb_to_postgres.py --init-schema --staging-dir data/staging/or_ccb
"""
...
log = logging.getLogger("load_or_ccb")
    ...
    *,
    ...
    with conn.cursor() as cur:
        for row in iter_csv(path):
            ...
            state = (row.get("state") or "OR")[:2].upper() or "OR"
...
def print_db_summary(conn) -> None:
    ...
    log.info("DB or_ccb licenses = %s", lic)
    log.info("DB by type:")
    for r in by_type:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Load staged Oregon CCB data into Postgres")
    parser.add_argument(
        "--staging-dir",
        type=Path,
        default=ROOT / "data" / "staging" / "or_ccb",
    )
    parser.add_argument(
        "--schema",
        type=Path,
    ...
    summary: dict[str, Any] = {
        ...
        "coverage_note": (
            "Oregon CCB Active Licenses — statewide contractor licensing. Bond/insurance as published."
        ),
    }
    ...
    try:
        with psycopg.connect(dsn) as conn:
            ...
            if not lic_path.exists():
                ...
                log.error(
                    "Run: python scripts/download_or_ccb.py && "
            ...
            batch_id = create_batch(
                ...
                checksum=checksum,
                notes=(
                    "load_or_ccb_to_postgres — Oregon CCB Active Licenses"
                ),
            )
    ...
    log.info("Oregon CCB load complete.")
    return 0
