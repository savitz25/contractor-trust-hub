#!/usr/bin/env python3
"""
Load staged Arizona ROC licenses into Supabase via PostgREST (service role).

Usage:
  python scripts/load_az_roc_via_supabase_rest.py --staging-dir data/staging/az_roc
  python scripts/load_az_roc_via_supabase_rest.py --staging-dir data/staging/az_roc --limit 500
"""
...
def main() -> int:
    ...
    print(f"Loading {len(rows)} AZ ROC licenses → {base}")
    ...
    batch_body = {
        ...
        "source_dataset": manifest.get("source_dataset") or "roc_current_active_contractors",
        ...
        "notes": manifest.get("notes") or "AZ ROC posting-list load via PostgREST",
    }
    _, raw, _ = rest(
    ...
    for r in rows:
        ...
        slug = slugify("az", external_key, display)
        home_state = (r.get("state") or "AZ")[:2].upper()
        ...
        licenses.append(
            {
                ...
                "county_name": county,
                "board_number": r.get("board_number") or "ROC",
    ...
    print("az_roc", headers.get("Content-Range") or headers.get("content-range"))
    print("DONE")
    return 0
