#!/usr/bin/env python3
"""
Load staged Texas TDLR specialty licenses into Postgres.

Idempotent upserts:
  - licenses on (source_system, external_key)
  - contractors on slug

Texas has no statewide GC license — this loader only handles TDLR specialty staging
from ingest/adapters/tx_tdlr.py.

Usage:
  python scripts/load_tx_tdlr_to_postgres.py --staging-dir data/staging/tx_tdlr
  python scripts/load_tx_tdlr_to_postgres.py --staging-dir data/staging/tx_tdlr --limit 1000
  python scripts/load_tx_tdlr_to_postgres.py --init-schema --staging-dir data/staging/tx_tdlr
"""
...
log = logging.getLogger("load_tx_tdlr")
...
def print_db_summary(conn) -> None:
    ...
    log.info("DB tx_tdlr licenses = %s", lic)
    log.info("DB by type:")
    for r in by_type:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Load staged TX TDLR data into Postgres")
    parser.add_argument(
        "--staging-dir",
        type=Path,
        default=ROOT / "data" / "staging" / "tx_tdlr",
    )
    parser.add_argument(
        "--schema",
        type=Path,
    ...
    summary: dict[str, Any] = {
        ...
        "coverage_note": (
            "Texas TDLR specialty trades only — no statewide general contractor license."
        ),
    }
    ...
    try:
        with psycopg.connect(dsn) as conn:
            ...
            if not lic_path.exists():
                ...
                log.error(
                    "Run: python scripts/download_tx_tdlr.py && "
            ...
            batch_id = create_batch(
                ...
                checksum=checksum,
                notes=(
                    "load_tx_tdlr_to_postgres — TDLR specialty contractors only "
                    "(no statewide GC)"
                ),
            )
    ...
    log.info("Texas TDLR load complete.")
    return 0
