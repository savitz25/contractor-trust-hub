#!/usr/bin/env python3
"""
Load staged Arizona ROC current active contractors into Postgres.
...
Usage:
  ...
  python scripts/load_az_roc_to_postgres.py --staging-dir data/staging/az_roc --limit 1000
  python scripts/load_az_roc_to_postgres.py --init-schema --staging-dir data/staging/az_roc
"""
...
log = logging.getLogger("load_az_roc")
    ...
    *,
    ...
    with conn.cursor() as cur:
        for row in iter_csv(path):
            ...
            state = (row.get("state") or "AZ")[:2].upper() or "AZ"
            ...
            cur.execute(
                ...
                (
                    ...
                    row.get("source_board") or "ROC",
                    external_key,
                    (row.get("occupation_code") or "").upper() or "ROC",
...
def print_db_summary(conn) -> None:
    ...
    log.info("DB az_roc licenses = %s", lic)
    log.info("DB by type:")
    for r in by_type:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Load staged Arizona ROC data into Postgres")
    parser.add_argument(
        "--staging-dir",
        type=Path,
        default=ROOT / "data" / "staging" / "az_roc",
    )
    parser.add_argument(
        "--schema",
        type=Path,
    ...
    summary: dict[str, Any] = {
        ...
        "coverage_note": (
            "Arizona ROC current active contractors — statewide licensing via official posting list."
        ),
    }
    ...
    try:
        with psycopg.connect(dsn) as conn:
            ...
            if not lic_path.exists():
                ...
                log.error(
                    "Run: python scripts/download_az_roc.py (or place CSV in data/raw/az_roc/) && "
                    "python -m ingest.adapters.az_roc --input-dir data/raw/az_roc"
                )
                return 1
            ...
            batch_id = create_batch(
                ...
                checksum=checksum,
                notes=(
                    "load_az_roc_to_postgres — Arizona ROC current active posting list"
                ),
            )
    ...
    log.info("Arizona ROC load complete.")
    return 0
