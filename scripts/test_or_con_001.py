...
class OrConTests(unittest.TestCase):
    def test_frozen_counts(self):
        ...
        self.assertEqual(SNAP["identity"]["EXACT_CCB_LICENSE"], 45501)
        self.assertEqual(SNAP["identity"]["EXACT_BCD_LICENSE"], 47959)
        self.assertEqual(SNAP["identity"]["EXACT_SOURCE_NATIVE_CROSSWALK"], 0)
        self.assertEqual(SNAP["identity"]["NAME_ONLY_UNSAFE"], 0)
        self.assertEqual(SNAP["adverse_publication"]["EXACT_PROFILE_ATTACHMENTS"], 0)
        self.assertEqual(SNAP["adverse_publication"]["REVIEW_REQUIRED"], 0)
        self.assertIsNone(SNAP["adverse_publication"]["UNRESOLVED"])
        self.assertEqual(SNAP["adverse_publication"]["INTERNAL_ONLY"], 0)
        self.assertEqual(SNAP["adverse_publication"]["PUBLICATION_PENDING"], 0)
