#!/usr/bin/env python3
"""
Load staged Texas TSBPE plumbing licenses into Postgres.

Idempotent upserts:
  - licenses on (source_system, external_key)
  - contractors on slug

Texas has no statewide GC license — this loader only handles TSBPE plumbing staging
from ingest/adapters/tx_tsbpe.py (Responsible Master Plumber + Master by default).

Usage:
  python scripts/load_tx_tsbpe_to_postgres.py --staging-dir data/staging/tx_tsbpe
  python scripts/load_tx_tsbpe_to_postgres.py --staging-dir data/staging/tx_tsbpe --limit 1000
  python scripts/load_tx_tsbpe_to_postgres.py --init-schema --staging-dir data/staging/tx_tsbpe
"""
...
log = logging.getLogger("load_tx_tsbpe")
    ...
    *,
    ...
    with conn.cursor() as cur:
        for row in iter_csv(path):
            ...
            state = (row.get("state") or "TX")[:2].upper() or "TX"
            ...
            cur.execute(
                ...
                (
                    ...
                    row.get("source_board") or "TSBPE",
                    external_key,
                    (row.get("occupation_code") or "").upper() or "TX",
...
def print_db_summary(conn) -> None:
    ...
    log.info("DB tx_tsbpe licenses = %s", lic)
    log.info("DB by type:")
    for r in by_type:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Load staged TX TSBPE plumbing data into Postgres")
    parser.add_argument(
        "--staging-dir",
        type=Path,
        default=ROOT / "data" / "staging" / "tx_tsbpe",
    )
    parser.add_argument(
        "--schema",
        type=Path,
    ...
    summary: dict[str, Any] = {
        ...
        "coverage_note": (
            "Texas TSBPE plumbing only — Responsible Master Plumbers can contract with the public. No statewide GC."
        ),
    }
    ...
    try:
        with psycopg.connect(dsn) as conn:
            ...
            if not lic_path.exists():
                ...
                log.error(
                    "Run: python scripts/download_tx_tsbpe.py && "
            ...
            batch_id = create_batch(
                ...
                checksum=checksum,
                notes=(
                    "load_tx_tsbpe_to_postgres — TSBPE plumbing (RMP/Master) "
                    "(no statewide GC)"
                ),
            )
    ...
    log.info("Texas TSBPE load complete.")
    return 0
