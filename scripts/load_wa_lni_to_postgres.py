#!/usr/bin/env python3
"""
Load staged Washington L&I contractor licenses into Postgres.
...
Usage:
  ...
  python scripts/load_wa_lni_to_postgres.py --staging-dir data/staging/wa_lni --limit 1000
  python scripts/load_wa_lni_to_postgres.py --init-schema --staging-dir data/staging/wa_lni
"""
...
log = logging.getLogger("load_wa_lni")
    ...
    *,
    ...
    with conn.cursor() as cur:
        for row in iter_csv(path):
            ...
            state = (row.get("state") or "WA")[:2].upper() or "WA"
            ...
            cur.execute(
                ...
                (
                    ...
                    row.get("source_board") or "LNI",
                    external_key,
                    (row.get("occupation_code") or "").upper() or "CC",
...
def print_db_summary(conn) -> None:
    ...
    log.info("DB wa_lni licenses = %s", lic)
    log.info("DB by type:")
    for r in by_type:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Load staged Washington L&I data into Postgres")
    parser.add_argument(
        "--staging-dir",
        type=Path,
        default=ROOT / "data" / "staging" / "wa_lni",
    )
    parser.add_argument(
        "--schema",
        type=Path,
    ...
    summary: dict[str, Any] = {
        ...
        "coverage_note": (
            "Washington L&I contractor licenses — statewide. Status/type/specialty as published. No bond/insurance in this feed."
        ),
    }
    ...
    try:
        with psycopg.connect(dsn) as conn:
            ...
            if not lic_path.exists():
                ...
                log.error(
                    "Run: python scripts/download_wa_lni.py && "
            ...
            batch_id = create_batch(
                ...
                checksum=checksum,
                notes=(
                    "load_wa_lni_to_postgres — Washington L&I contractor licenses"
                ),
            )
    ...
    log.info("Washington L&I load complete.")
    return 0
