...
test('actual geography controls do not prefetch a correction or broader execution',async()=>{const {GeographyNotice}=await import('../components/ask/GeographyNotice');for(const q of [typo,'HVAC contractors in Austin Texas']){const requirement=discovery(q).geographyRequirement;const element=GeographyNotice({requirement,query:q,route:'/search'});let choices=0;function walk(e:any){if(!e||typeof e!=='object')return;if(Array.isArray(e)){e.forEach(walk);return;}if(typeof e.props?.href==='string'&&e.props.href.includes('geoAction=')){choices++;assert.equal(e.props.prefetch,false);}walk(e.props?.children);}walk(element);assert.ok(choices>0);}});

test('accepted correction can be followed by explicit state broadening without losing its history',async()=>{const {GeographyNotice}=await import('../components/ask/GeographyNotice');const corrected=ask(typo,{geoAction:'correct',geoChoice:'broward'});const element=GeographyNotice({requirement:corrected.geographyRequirement,query:typo,overrides:planToOverrides(corrected)});let href='';function walk(e:any){if(!e||typeof e!=='object')return;if(Array.isArray(e)){e.forEach(walk);return;}if(typeof e.props?.href==='string'&&e.props.href.includes('geoAction=broaden'))href=e.props.href;walk(e.props?.children);}walk(element);assert.ok(href);const request=readAskRequest(Object.fromEntries(new URL(href,'https://www.contractortrusthub.com').searchParams));const broadened=ask(request.query,request.overrides);assert.equal(broadened.executable,true);assert.equal(broadened.geographyRequirement?.executionOutcome,'USER_APPROVED_RELAXATION');assert.equal(broadened.geographyRequirement?.requestedCounty,'Browrd');assert.equal(broadened.geography.countySlug,null);assert.deepEqual(askWhere(broadened),askWhere(ask('roofers in Florida')));});

test('invalid prior correction cannot authorize typo broadening',()=>{assert.equal(ask(typo,{geoAction:'broaden',geoChoice:'fl',geoCorrection:'palm-beach'}).executable,false);assert.equal(ask('roofers in Bradford County Florida',{geoAction:'broaden',geoChoice:'fl',geoCorrection:'broward'}).executable,false);});
