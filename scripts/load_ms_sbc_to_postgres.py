#!/usr/bin/env python3
"""
Load staged Mississippi MSBOC contractor licenses into Postgres.
...
Usage:
  ...
  python scripts/load_ms_sbc_to_postgres.py --staging-dir data/staging/ms_sbc --limit 1000
  python scripts/load_ms_sbc_to_postgres.py --init-schema --staging-dir data/staging/ms_sbc
"""
...
log = logging.getLogger("load_ms_sbc")
    ...
    *,
    ...
    with conn.cursor() as cur:
        for row in iter_csv(path):
            ...
            state = (row.get("state") or "MS")[:2].upper() or "MS"
            ...
            cur.execute(
                ...
                (
                    ...
                    row.get("source_board") or "MSBOC",
                    external_key,
                    (row.get("occupation_code") or "").upper() or "COM",
...
def print_db_summary(conn) -> None:
    with conn.cursor() as cur:
        ...
        cur.execute(
            """
            SELECT COALESCE(class_code, '(none)'), COUNT(*)
            FROM licenses WHERE source_system = %s
            GROUP BY 1 ORDER BY 2 DESC LIMIT 12
            """,
            (SOURCE_SYSTEM,),
    ...
    log.info("DB ms_sbc licenses = %s", lic)
    log.info("DB by type:")
    for r in by_type:
    ...
    for r in by_status:
        log.info("  %s: %s", r[0], r[1])
    log.info("DB by class_code:")
    for r in by_class:
        log.info("  %s: %s", r[0], r[1])


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Load staged Mississippi MSBOC data into Postgres")
    parser.add_argument(
        "--staging-dir",
        type=Path,
        default=ROOT / "data" / "staging" / "ms_sbc",
    )
    parser.add_argument(
        "--schema",
        type=Path,
    ...
    summary: dict[str, Any] = {
        ...
        "coverage_note": (
            "Mississippi MSBOC official public consolidated list — commercial / residential "
            "as published. No bond, insurance, or discipline on the list view."
        ),
    }
    ...
    try:
        with psycopg.connect(dsn) as conn:
            ...
            if not lic_path.exists():
                ...
                log.error(
                    "Run: python scripts/download_ms_sbc.py && "
            ...
            batch_id = create_batch(
                ...
                checksum=checksum,
                notes=(
                    "load_ms_sbc_to_postgres — Mississippi MSBOC official public list"
                ),
            )
    ...
    log.info("Mississippi MSBOC load complete.")
    return 0
