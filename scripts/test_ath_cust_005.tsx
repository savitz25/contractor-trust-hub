...
const record = { contractVersion: 1, hub: "contractor", nativeProfileId: ID, managed: true, source: "BUSINESS_SUPPLIED",
  ...
  fields: { description: "Safe business description", website: "https://example.com", public_phone: "555-555-1212", founded_year: "1998" },
...
test("public contract accepts only the exact UUID and public allowlist", () => {
  ...
  assert.equal(parsePublicBusinessProfile({ ...record, claimantEmail: "private@example.com" }, ID), null);
  ...
  assert.equal(parsePublicBusinessProfile({ ...record, fields: { description: "<script>alert(1)</script>" } }, ID), null);
  assert.equal(parsePublicBusinessProfile({ ...record, fields: { contact_context: "private operator note" } }, ID), null);
});
...
test("public overlay escapes text and keeps self-reported service area distinct", () => {
  ...
  assert.match(html, /Information supplied by the business/); assert.match(html, /Profile managed by an authorized representative/);
