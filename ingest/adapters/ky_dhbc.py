"""
Kentucky DHBC specialty-license adapter (Phase 0).

Kentucky has NO statewide general contractor license.
This adapter only normalizes official DHBC search-list fields.

Usage:
  python -m ingest.adapters.ky_dhbc --input data/samples/ky_dhbc_contractor_sample.csv
"""

from __future__ import annotations

import argparse
import csv
import hashlib
import json
import re
from datetime import datetime, timezone
from pathlib import Path
from typing import Any, Iterator

SOURCE_SYSTEM = "ky_dhbc"
SOURCE_BOARD = "DHBC"
SOURCE_URL = "https://dhbc.ky.gov/Search/HBC_List_Licensees.aspx"

LICENSE_OUT_FIELDS = [
    "source_system",
    "source_board",
    "external_key",
    "occupation_code",
    "occupation_description",
    "license_number",
    "class_code",
    "licensee_name_raw",
    "dba_name_raw",
    "primary_status",
    "secondary_status",
    "status_normalized",
    "original_licensure_date",
    "effective_date",
    "expiration_date",
    "address_line_1",
    "address_line_2",
    "address_line_3",
    "city",
    "state",
    "postal_code",
    "county_code",
    "county_name",
    "board_number",
    "raw_payload_json",
]

CONTRACTOR_SEED_FIELDS = [
    "slug",
    "display_name",
    "legal_name",
    "dba_name",
    "home_state",
    "primary_city",
    "primary_county",
    "license_external_key",
]

# Firm-level types recommended for a later Verify v1.
FIRM_TYPES = {
    "CONTRACTOR ELECTRICIAN-BUSINESS": ("ELEC", "ELEC"),
    "MASTER HVAC CONTRACTOR": ("HVAC", "HVAC"),
    "MASTER PLUMBER": ("PLB", "PLB"),
    "FIRE PROTECTION SPRINKLER SYSTEMS CONTRACTORS LICENSE": ("FIRE", "FIRE"),
    "FIRE PROTECTION CHEMICAL SYSTEMS CONTRACTORS LICENSE": ("FIRE", "FIRE"),
    "FIRE PROTECTION RANGEHOOD SYSTEMS CONTRACTORS LICENSE": ("FIRE", "FIRE"),
    "MASTER ELECTRICIAN": ("MEL", "ELE"),
}

STATUS_NORMALIZED = {
    "ACTIVE": "active",
    "EXPIRED": "expired",
    "INACTIVE": "inactive",
    "NOT ACTIVE": "inactive",
    "SUSPENDED": "suspended",
    "REVOKED": "revoked",
    "CANCELLED": "other",
    "CLOSED": "other",
    "DELINQUENT": "other",
    "PENDING": "other",
}

COL_ALIASES = {
    "licenseename": "LicenseeName",
    "licensee": "LicenseeName",
    "name": "LicenseeName",
    "licensetype": "LicenseType",
    "license type": "LicenseType",
    "dbaname": "DbaName",
    "dba name": "DbaName",
    "dba": "DbaName",
    "licensenumber": "LicenseNumber",
    "license number": "LicenseNumber",
    "status": "Status",
    "issueddate": "IssuedDate",
    "issued date": "IssuedDate",
    "expirationdate": "ExpirationDate",
    "expiration date": "ExpirationDate",
}


def _clean(value: str | None) -> str:
    if value is None:
        return ""
    return str(value).strip()


def _canon_row(row: dict[str, str]) -> dict[str, str]:
    out: dict[str, str] = {}
    for k, v in row.items():
        key = COL_ALIASES.get(_clean(k).lower(), _clean(k))
        out[key] = _clean(v) if isinstance(v, str) else ""
    return out


def _sha256_file(path: Path) -> str:
    h = hashlib.sha256()
    with path.open("rb") as f:
        for chunk in iter(lambda: f.read(1024 * 1024), b""):
            h.update(chunk)
    return h.hexdigest()


def _parse_date(value: str | None) -> str:
    v = _clean(value)
    if not v:
        return ""
    for fmt in ("%m/%d/%Y", "%Y-%m-%d", "%m-%d-%Y"):
        try:
            return datetime.strptime(v, fmt).date().isoformat()
        except ValueError:
            continue
    return ""


def _normalize_status(raw: str) -> str:
    return STATUS_NORMALIZED.get(_clean(raw).upper(), "other" if raw else "other")


def _type_codes(license_type: str) -> tuple[str, str, str]:
    published = _clean(license_type)
    mapped = FIRM_TYPES.get(published.upper())
    if mapped:
        occ, family = mapped
        return occ, published, family
    return "OTH", published or "Kentucky DHBC credential", "OTH"


def _slugify(*parts: str, max_len: int = 120) -> str:
    raw = "-".join(p for p in parts if p)
    s = re.sub(r"[^a-zA-Z0-9]+", "-", raw.lower()).strip("-")
    return (s or "unknown")[:max_len]


def _iter_csv(path: Path) -> Iterator[dict[str, str]]:
    with path.open("r", encoding="utf-8", errors="replace", newline="") as f:
        reader = csv.DictReader(f)
        for row in reader:
            yield _canon_row({k: _clean(v) if isinstance(v, str) else "" for k, v in row.items()})


def normalize_row(row: dict[str, str]) -> dict[str, str] | None:
    lic = _clean(row.get("LicenseNumber"))
    name = _clean(row.get("LicenseeName"))
    if not lic or not name:
        return None
    occ, published, family = _type_codes(row.get("LicenseType") or "")
    status = _clean(row.get("Status")) or "UNKNOWN"
    dba = _clean(row.get("DbaName"))
    # Production load uses KY-DHBC:{number} (not family-prefixed).
    external_key = f"KY-DHBC:{lic.upper()}"
    payload = {k: v for k, v in row.items() if v}
    display = dba or name

    return {
        "source_system": SOURCE_SYSTEM,
        "source_board": SOURCE_BOARD,
        "external_key": external_key,
        "occupation_code": occ,
        "occupation_description": published,
        "license_number": lic.upper(),
        "class_code": occ,
        "licensee_name_raw": name,
        "dba_name_raw": dba,
        "primary_status": status,
        "secondary_status": f"Type {published}" if published else "",
        "status_normalized": _normalize_status(status),
        "original_licensure_date": _parse_date(row.get("IssuedDate")),
        "effective_date": _parse_date(row.get("IssuedDate")),
        "expiration_date": _parse_date(row.get("ExpirationDate")),
        "address_line_1": "",
        "address_line_2": "",
        "address_line_3": "",
        "city": "",
        "state": "KY",
        "postal_code": "",
        "county_code": "",
        "county_name": "",
        "board_number": "DHBC",
        "raw_payload_json": json.dumps(payload, ensure_ascii=False),
        "_display": display,
        "_slug": _slugify(external_key, display),
    }


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Normalize Kentucky DHBC specialty lists")
    p.add_argument("--input", type=Path, required=True)
    p.add_argument("--out-dir", type=Path, default=Path("data/staging/ky_dhbc"))
    p.add_argument("--limit", type=int, default=None)
    args = p.parse_args(argv)

    if not args.input.exists():
        raise SystemExit(f"Missing input: {args.input}")

    args.out_dir.mkdir(parents=True, exist_ok=True)
    rows: list[dict[str, str]] = []
    skipped = 0
    collisions = 0
    seen: set[str] = set()
    for raw in _iter_csv(args.input):
        row = normalize_row(raw)
        if row is None:
            skipped += 1
            continue
        if row["external_key"] in seen:
            collisions += 1
            skipped += 1
            continue
        seen.add(row["external_key"])
        rows.append(row)
        if args.limit is not None and len(rows) >= args.limit:
            break

    lic_path = args.out_dir / "licenses_normalized.csv"
    seed_path = args.out_dir / "contractors_seed.csv"
    with lic_path.open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=LICENSE_OUT_FIELDS, extrasaction="ignore")
        w.writeheader()
        for row in rows:
            w.writerow(row)
    with seed_path.open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=CONTRACTOR_SEED_FIELDS)
        w.writeheader()
        for row in rows:
            w.writerow(
                {
                    "slug": row["_slug"],
                    "display_name": row["_display"],
                    "legal_name": row["licensee_name_raw"],
                    "dba_name": row.get("dba_name_raw") or "",
                    "home_state": "KY",
                    "primary_city": "",
                    "primary_county": "",
                    "license_external_key": row["external_key"],
                }
            )

    by_type: dict[str, int] = {}
    by_status: dict[str, int] = {}
    for row in rows:
        by_type[row["occupation_code"]] = by_type.get(row["occupation_code"], 0) + 1
        by_status[row["status_normalized"]] = by_status.get(row["status_normalized"], 0) + 1

    manifest: dict[str, Any] = {
        "source_system": SOURCE_SYSTEM,
        "source_dataset": "dhbc_licensee_search_sample",
        "source_url": SOURCE_URL,
        "source_file": str(args.input).replace("\\", "/"),
        "source_sha256": _sha256_file(args.input),
        "extracted_at": datetime.now(timezone.utc).isoformat(),
        "row_count_licenses": len(rows),
        "skipped": skipped,
        "duplicate_keys": collisions,
        "by_type": dict(sorted(by_type.items(), key=lambda kv: -kv[1])),
        "by_status": dict(sorted(by_status.items(), key=lambda kv: -kv[1])),
        "coverage_note": (
            "Kentucky DHBC specialty credentials only. No statewide general contractor license. "
            "No bond, insurance, or discipline on the official list view."
        ),
        "outputs": {
            "licenses_normalized.csv": str(lic_path).replace("\\", "/"),
            "contractors_seed.csv": str(seed_path).replace("\\", "/"),
        },
    }
    (args.out_dir / "batch_manifest.json").write_text(
        json.dumps(manifest, indent=2), encoding="utf-8"
    )
    print(json.dumps(manifest, indent=2))
    print(f"OK: {len(rows)} licenses (skipped={skipped}, duplicate_keys={collisions})")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
